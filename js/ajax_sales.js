...
$("#addmore").on('click',function(){
	...
	html += '<td><input type="text" data-type="productCode" name="product_id[]" id="itemNo_'+i+'" class="form-control autocomplete_txt" autocomplete="off">	<input type="hidden" data-type="productName" name="product_name[]" id="itemName_'+i+'" class="form-control autocomplete_txt" autocomplete="off"></td>';
	html += '<td><input type="text" name="price[]" id="price_'+i+'" class="form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
	html += '<td><input type="text" data-type="productP_Blance" name="previous_balance[]" id="previous_balance_'+i+'" class="form-control autocomplete_txt" autocomplete="off" readonly></td>';
	html += '<td><input type="text" name="quantity[]" id="quantity_'+i+'" class="form-control changesNo selectqty" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
...
item_qty = $(this).val();
totalqty = Number(totalqty) + Number(item_qty);
});
...
$(document).on('change keyup blur','.selectqty',function(){
	$('#totalsaleqty').val( totalqty);
	totalqty();
});
...
$(document).on('focus','.autocomplete_txt',function(){
	...
	if(type =='productP_Blance')autoTypeNo=2;
	...
	$(this).autocomplete({
		...
		select: function( event, ui ) {
			...
			$('#previous_balance_'+id[1]).val(names[3]);
...
$(document).on('change keyup blur','.changesNo',function(){
	...
	previous_balance = $('#previous_balance_'+id[1]).val();	
	if(quantity>previous_balance){
		$('#previous_balance_'+id[1]).css("border","1px solid red");
		
	}else{
		$('#previous_balance_'+id[1]).css("border","1px solid #ccc");
	}
...
//});
	...
	if( quantity!='' && price !='' && quantity<=previous_balance) $('#total_'+id[1]).val( (parseFloat(price)*parseFloat(quantity)).toFixed(2) );	
	calculateTotal();
});
...
$(document).on('change keyup blur','#totaldiscountAmount',function(){
	calculateTotal();
});
//total price calculation 
function calculateTotal(){
	subTotal = 0 ; total = 0; var totaldiscountAmount = 0;
	...
	if(totaldiscountAmount != '' && typeof(totaldiscountAmount) != "undefined" ){
		///taxAmount = subTotal * ( parseFloat(tax) /100 );
		//$('#taxAmount').val(taxAmount.toFixed(2));
...
function calculateAmountDue(){
	...
	if(amountPaid != '' && typeof(amountPaid) != "undefined" ){
		...
		grandTotal = Number(amountDue) + Number(customerBalance);
		$('#grand_total').val(grandTotal);
		
	}else{
		total = parseFloat(total).toFixed(2);
		$('.amountDue').val( total);
		grandTotal = Number(total) + Number(customerBalance);
		$('#grand_total').val(grandTotal);
		//console.log(total);
