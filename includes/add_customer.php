...
if ( isset( $_POST[ 'submit' ] ) ) {
	...
	if ( empty( $error ) ) {
		$data_post = array('code' => $max_id->code+1, 'name' => $name, 'address' => $Address, 'city' => $city, 'contact_person' => $cntct_person, 'contact_no' => $cntct_no, 'limit_amount' => $lmit_amount, 'credit_limit_days' => $credit_lmt_dys, 'gl_acct' => $gl_accnt );
		$recodes = $conf->insert( $data_post, CUSTOMERS );
		if ( $recodes == true ) {
				...
				<form id="demo-form2" data-parsley-validate="" method="post" class="form-horizontal form-label-left" novalidate enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
							<input type="text" class="form-control" name="custmer_id" disabled value="<?php echo $max_id->code+1;?>">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
							<input type="number" class="form-control" name="cntct_person" required data-required="true" value="<?=$cntct_person?>" >
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-md-3 col-sm-3 col-xs-12" for="custom-id">Contact # <span class="red">*</span></label>
						<div class="col-md-6 col-sm-6 col-xs-12">
							<input type="number" class="form-control" name="cntct_no" required data-required="true" value="<?=$cntct_no?>" >
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-md-3 col-sm-3 col-xs-12" for="custom-id">Limit Amount: </label>
						<div class="col-md-6 col-sm-6 col-xs-12">
							<input type="number" class="form-control" name="lmit_amount" value="<?=$lmit_amount?>" >
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-md-3 col-sm-3 col-xs-12" for="custom-id">Credit Limit Days: </label>
						<div class="col-md-6 col-sm-6 col-xs-12">
							<input type="number" class="form-control" name="crdt_lmt_dys" value="<?=$credit_lmt_dys?>" >
						</div>
					</div>
					<div class="form-group">
